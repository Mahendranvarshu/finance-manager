    ...
    <div class="col-md-8 mx-auto">
        <div class="card shadow-sm">
            <div class="card-header bg-white">
                <h4 class="mb-0"><i class="bi bi-plus-circle"></i> Add New Collection</h4>
            </div>
            <div class="card-body">
                <form action="{{ route('collections.store') }}" method="POST">
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('party_id') is-invalid @enderror" 
                                ...
                                @foreach($parties as $party)
                                    <option value="{{ $party->id }}" {{ old('party_id', request('party_id')) == $party->id ? 'selected' : '' }}>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" step="0.01" class="form-control @error('amount_collected') is-invalid @enderror" 
                                   id="amount_collected" name="amount_collected" value="{{ old('amount_collected') }}" required>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" class="form-control @error('day_number') is-invalid @enderror" 
                                   id="day_number" name="day_number" value="{{ old('day_number') }}">
                            @error('day_number')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <small class="text-muted">Auto-calculated if blank</small>
                        </div>
                    </div>
                    ...
                    <div class="alert alert-info">
                        ...
                        Loan Amount: ₹{{ number_format($selectedParty->loan_amount, 2) }}<br>
                        Interest: ₹{{ number_format($selectedParty->interest_amount ?? 0, 2) }}<br>
                        Daily Amount: ₹{{ number_format($selectedParty->daily_amount ?? 0, 2) }}<br>
                        Collected: ₹{{ number_format($selectedParty->collections->sum('amount_collected'), 2) }}
                    </div>
                    @endif
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="number" step="0.01" class="form-control @error('remaining_amount') is-invalid @enderror" 
                                   id="remaining_amount" name="remaining_amount" value="{{ old('remaining_amount') }}" readonly>
